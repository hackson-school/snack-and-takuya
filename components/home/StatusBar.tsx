...
export function StatusBar({ calorieStock, cavityRisk }: StatusBarProps) {
  ...
  return (
    <div className="w-full bg-zinc-900 border-x-4 border-b-4 border-yellow-400 p-2 space-y-1.5 select-none shadow-md shrink-0">
      {/* カロリーバー */}
      <div className="space-y-0.5">
        <div className="flex items-center justify-between text-[11px] font-bold font-game">
          <div className="flex items-center gap-1.5 text-orange-400">
            <Flame className="w-3 h-3" />
            <span>CALORIE STOCK</span>
          </div>
          <div className="text-zinc-200">
            <span className={`text-xs ${isCalorieWarning ? 'text-red-400 font-extrabold animate-pulse-fast' : 'text-yellow-300'}`}>
              {calorieStock}
            </span>
            <span className="text-zinc-400 text-[9px]"> / 2000 kcal</span>
          </div>
        </div>

        {/* ゲージ外枠 */}
        <div className="relative h-3 w-full bg-zinc-950 rounded-xs border-2 border-zinc-700 p-0.5 overflow-hidden">
          <div
      ...
      <div className="space-y-0.5">
        <div className="flex items-center justify-between text-[11px] font-bold font-game">
          ...
          <div className="text-zinc-200">
            <span className={`text-xs ${isCavityWarning ? 'text-red-400 font-extrabold animate-pulse-fast' : 'text-emerald-400'}`}>
              {cavityRisk}
            </span>
            <span className="text-zinc-400 text-[9px]"> / 100 PTS</span>
          </div>
        </div>
